...
extern char *_xor_secret;
extern int _xor_secret_len;
...
static const char usage_message[] =
	...
	"                  AGENT user-agent\n"
	"--socks-proxy s [p] [up] : Connect to remote host through a Socks5 proxy "
	"at\n"
	...
	"--ifconfig-nowarn : Don't warn if the --ifconfig option on this side of "
	"the\n"
	...
	"                  is established.  Multiple routes can be specified.\n"
	"                  gateway default: taken from --route-ipv6-gateway or "
	"'remote'\n"
	"                  in --ifconfig-ipv6\n"
	"--route-gateway gw|'dhcp' : Specify a default gateway for use with "
	"--route.\n"
	"--route-ipv6-gateway gw : Specify a default gateway for use with "
	"--route-ipv6.\n"
	"--route-metric m : Specify a default metric for use with --route.\n"
	"--route-delay n [w] : Delay n seconds after connection initiation before\n"
	"                  adding routes (may be 0).  If not specified, routes "
	"will\n"
	...
	"                  commands to redirect all outgoing IP traffic through "
	"the\n"
	"                  VPN.  Add 'local' flag if both "
PACKAGE_NAME
" servers are directly\n"
"                  connected via a common subnet, such as with WiFi.\n"
"                  Add 'def1' flag to set default route using using "
"0.0.0.0/1\n"
"                  and 128.0.0.0/1 rather than 0.0.0.0/0.  Add "
"'bypass-dhcp'\n"
"                  flag to add a direct route to DHCP server, bypassing "
"tunnel.\n"
...
"Send\n"
"                  ping once every n seconds, restart if ping not "
"received\n"
"                  for m seconds.\n"
...
"                  (default TUN=0 TAP=%d).\n"
"--link-mtu n    : Take the TCP/UDP device MTU to be n and derive the tun "
"MTU\n"
"                  from it.\n"
...
"                  until after TCP/UDP connection establishment with "
"peer.\n"
...
"                  See --daemon above for a description of the 'name' "
"parm.\n"
...
"--gremlin mask  : Special stress testing mode (for debugging only).\n"
...
#endif
    "--management-client : Management interface will connect as a TCP client "
    "to\n"
    ...
    "directive.\n"
    "--management-hold : Start " PACKAGE_NAME
    " in a hibernating state, until a client\n"
    "                    of the management interface explicitly starts it.\n"
    "--management-signal : Issue SIGUSR1 when management disconnect event "
    "occurs.\n"
    "--management-forget-disconnect : Forget passwords when management "
    "disconnect\n"
    "                                 event occurs.\n"
    "--management-up-down : Report tunnel up/down events to management "
    "interface.\n"
    "--management-log-cache n : Cache n lines of log file history for usage\n"
    "                  by the management channel.\n"
#if UNIX_SOCK_SUPPORT
    "--management-client-user u  : When management interface is a unix socket, "
    "only\n"
    "                              allow connections from user u.\n"
    "--management-client-group g : When management interface is a unix socket, "
    "only\n"
    "                              allow connections from group g.\n"
#endif
#ifdef MANAGEMENT_DEF_AUTH
    "--management-client-auth : gives management interface client the "
    "responsibility\n"
    "                           to authenticate clients after their client "
    "certificate\n"
    "			      has been verified.\n"
#endif
#ifdef MANAGEMENT_PF
    "--management-client-pf : management interface clients must specify a "
    "packet\n"
...
"--vlan-pvid v   : Sets the Port VLAN Identifier. Defaults to 1.\n"
    ...
    "--auth-user-pass-verify cmd method: Query client for username/password "
    "and\n"
    ...
    "--ccd-exclusive : Refuse connection unless custom client config is "
    "found.\n"
    "--tmp-dir dir   : Temporary directory, used for --client-connect return "
    "file and plugin communication.\n"
    ...
    "timestamp\n"
    "                             older than n seconds. Run this check every "
    "t\n"
...
#if PORT_SHARE
    "--port-share host port [dir] : When run in TCP mode, proxy incoming "
    "HTTPS\n"
...
"recvd.\n"
"--reneg-pkts n  : Renegotiate data chan. key after n packets sent and "
"recvd.\n"
"--reneg-sec max [min] : Renegotiate data chan. key after at most max "
"(default=%d)\n"
"                  and at least min (defaults to 90%% of max on servers "
"and equal\n"
"                  to max on clients).\n"
...
"top\n"
"                  of the TLS control channel to hide the TLS "
"certificate,\n"
"                  provide basic post-quantum security and protect "
"against\n"
"                  attacks on the TLS stack and DoS attacks.\n"
"                  key (required) provides the pre-shared key file.\n"
"                  see --secret option for more info.\n"
"--tls-crypt-v2 key : For clients: use key as a client-specific tls-crypt "
"key.\n"
"                  For servers: use key to decrypt client-specific keys.  "
"For\n"
"                  key generation (--genkey tls-crypt-v2-client): use key "
"to\n"
...
"--remote-cert-eku oid : Require that the peer certificate was signed "
"with\n"
"                  explicit extended key usage. Extended key usage can be "
"encoded\n"
"                  as an object identifier or OpenSSL string "
"representation.\n"
...
"                  t = 'client' | 'server'.\n"
    ...
    "--pkcs11-protected-authentication [0|1] ... : Use PKCS#11 protected "
    "authentication\n"
    "                              path. Set for each provider.\n"
    "--pkcs11-private-mode hex ...   : PKCS#11 private key mode mask.\n"
    "                              0       : Try  to determine automatically "
    "(default).\n"
    "                              1       : Use Sign.\n"
    "                              2       : Use SignRecover.\n"
    "                              4       : Use Decrypt.\n"
...
"channel).\n"
    ...
    "                    offset: DHCP server address offset (> -256 and < "
    "256).\n"
    ...
    "--pause-exit         : When run from a console window, pause before "
    "exiting.\n"
    "--service ex [0|1]   : For use when " PACKAGE_NAME
    " is being instantiated by a\n"
    "                       service, and should not be used directly by "
    "end-users.\n"
    "                       ex is the name of an event object which, when\n"
    "                       signaled, will cause " PACKAGE_NAME
    " to exit.  A second\n"
    "                       optional parameter controls the initial state of "
    "ex.\n"
    "--show-net-up   : Show " PACKAGE_NAME
    ...
    "--show-net      : Show " PACKAGE_NAME
    "'s view of routing table and net adapter list.\n"
    "--show-valid-subnets : Show valid subnets for --dev tun emulation.\n"
    "--allow-nonadmin [TAP-adapter] : Allow " PACKAGE_NAME
...
"--tls-crypt)."
    ...
    "--show-pkcs11-ids [provider] [cert_private] : Show PKCS#11 available "
    "ids.\n"
#else
...
init_options(struct options *o, const bool init_gc)
{
  ...
  o->tuntap_options.dhcp_masq_offset =
...
#define SHOW_STR_INLINE(var)                                                   \
  SHOW_PARM(var, o->var##_inline ? "[INLINE]" : (o->var ? o->var : "[UNDEF]"), \
            "'%s'")
...
static char *
string_substitute(const char *src, int from, int to,
				  struct gc_arena *gc)
{
...
static uint8_t *
parse_hash_fingerprint(const char *str, int nbytes,
					   int msglevel, struct gc_arena *gc)
{
	int i;
	const char *cp = str;
	uint8_t *ret = (uint8_t*)
	gc_malloc(nbytes, true, gc);
	char term = 1;
	int byte;
	char bs[3];
	...
	{
		msg(msglevel,
			"hash fingerprint is different length than expected (%d bytes): %s",
			nbytes, str);
	}
	return ret;
}
...
static void show_dhcp_option_list(const char *name, const char *const *array,
                                  int len) {
  int i;
  for (i = 0; i < len; ++i) {
...
static void show_dhcp_option_addrs(const char *name, const in_addr_t *array,
                                   int len) {
  struct gc_arena gc = gc_new();
  int i;
  for (i = 0; i < len; ++i) {
    msg(D_SHOW_PARMS, "  %s[%d] = %s", name, i,
...
static void show_tuntap_options(const struct tuntap_options *o) {
  ...
  show_dhcp_option_list("DOMAIN-SEARCH", o->domain_search_list,
                        o->domain_search_list_len);
																														...
																														static void dhcp_option_dns6_parse(const char *parm, struct in6_addr *dns6_list,
                                   int *len, int msglevel) {
...
static void dhcp_option_address_parse(const char *name, const char *parm,
                                      in_addr_t *array, int *len,
                                      int msglevel) {
  if (*len >= N_DHCP_ADDR) {
    msg(msglevel, "--dhcp-option %s: maximum of %d %s servers can be specified",
        name, N_DHCP_ADDR, name);
  } else {
    if (ip_addr_dotted_quad_safe(parm)) /* FQDN -- IP address only */
    {
      ...
      msg(msglevel, "dhcp-option parameter %s '%s' must be an IP address", name,
          parm);
    }
  }
}
...
print_vlan_accept(enum vlan_acceptable_frames mode)
{
	switch (mode)
	{
		case VLAN_ONLY_TAGGED:return "tagged";

		case VLAN_ONLY_UNTAGGED_OR_PRIORITY:return "untagged";

		case VLAN_ALL:return "all";
	}
	return NULL;
}
...
static void show_p2mp_parms(const struct options *o) {
  ...
  msg(D_SHOW_PARMS, "  server_network = %s",
      print_in_addr_t(o->server_network, 0, &gc));
  msg(D_SHOW_PARMS, "  server_netmask = %s",
      print_in_addr_t(o->server_netmask, 0, &gc));
  msg(D_SHOW_PARMS, "  server_network_ipv6 = %s",
      print_in6_addr(o->server_network_ipv6, 0, &gc));
  SHOW_INT(server_netbits_ipv6);
  msg(D_SHOW_PARMS, "  server_bridge_ip = %s",
      print_in_addr_t(o->server_bridge_ip, 0, &gc));
  ...
  msg(D_SHOW_PARMS, "  ifconfig_pool_end = %s",
      print_in_addr_t(o->ifconfig_pool_end, 0, &gc));
...
static void option_iroute(struct options *o, const char *network_str,
                          const char *netmask_str, int msglevel) {
  struct iroute *ir;
  ...
  if (netmask_str) {
    const in_addr_t netmask =
        getaddr(GETADDR_HOST_ORDER, netmask_str, 0, NULL, NULL);
...
static void option_iroute_ipv6(struct options *o, const char *prefix_str,
                               int msglevel) {
...
show_settings(const struct options *o)
{
  ...
  }
  {
    int i;
    for (i = 0; i < MAX_PARMS; i++) {
      SHOW_PARM(pkcs11_cert_private,
                o->pkcs11_cert_private[i] ? "ENABLED" : "DISABLED", "%s");
    }
  }
...
parse_http_proxy_override(const char *server, const char *port,
                          const char *flags, const int msglevel,
                          struct gc_arena *gc) {
  if (server && port) {
...
static void options_postprocess_http_proxy_override(struct options *o) {
  ...
  if (succeed) {
    ...
    msg(M_WARN, "Note: option http-proxy-override ignored because no TCP-based "
                "connection profiles are defined");
  }
}
...
alloc_connection_entry(struct options *options,
					   const int msglevel)
{
	...
	{
		msg(msglevel, "Maximum number of 'connection' options (%d) exceeded",
...
alloc_remote_entry(struct options *options,
				   const int msglevel)
{
	...
	if (l->len >= CONNECTION_LIST_SIZE)
	{
		msg(msglevel, "Maximum number of 'remote' options (%d) exceeded",
			CONNECTION_LIST_SIZE);
		return NULL;
	}
...
static void
connection_entry_load_re(struct connection_entry *ce,
...
static void
connection_entry_preload_key(const char **key_file,
							 bool *key_inline,
							 struct gc_arena *gc)
{
							  ...
							  const struct connection_entry *ce)
{
   ...
   */
	...
	if (proto_is_net(ce->proto) && string_defined_equal(ce->local, ce->remote) &&
   ...
   */
      ...
      (options->management_flags || options->management_write_peer_info_file ||
       options->management_log_history_cache !=
           defaults.management_log_history_cache)) {
    msg(M_USAGE, "--management is not specified, however one or more options "
                 "which modify the behavior of --management were specified");
  }

  if ((options->management_client_user || options->management_client_group) &&
      !(options->management_flags & MF_UNIX_SOCK)) {
    msg(M_USAGE, "--management-client-(user|group) can only be used on unix "
                 "domain sockets");
  }
...
#define USAGE_VALID_SERVER_PROTOS                                              \
  ...
  "--proto values of udp, tcp-server, tcp4-server, or tcp6-server"
     ...
     */
		...
		{
			msg(M_USAGE, "--connect-freq only works with --mode server --proto udp.  "
						 "Try --max-clients instead.");
		}
		if (!(dev==DEV_TYPE_TAP ||
			(dev==DEV_TYPE_TUN && options->topology==TOP_SUBNET)) &&
		...
		}
		{
			const bool ccnr =
				(options->auth_user_pass_verify_script ||
					PLUGIN_OPTION_LIST(options) || MAN_CLIENT_AUTH_ENABLED(options));
   ...
   */
	if (!options->replay && (options->replay_window!=defaults.replay_window ||
		options->replay_time!=defaults.replay_time))
	{
		msg(M_USAGE, "--replay-window doesn't make sense when replay protection is "
					 "disabled with --no-replay");
	}
   ...
   */
	if (options->tls_server + options->tls_client +
		(options->shared_secret_file!=NULL) >
		1)
	{
	...
	if (options->tls_server || options->tls_client)
	{
      ...
      if (options->pkcs11_id_management && options->pkcs11_id != NULL) {
        msg(M_USAGE, "Parameter --pkcs11-id cannot be used when "
                     "--pkcs11-id-management is also specified.");
      }
      if (!options->pkcs11_id_management && options->pkcs11_id == NULL) {
        msg(M_USAGE, "Parameter --pkcs11-id or --pkcs11-id-management should "
                     "be specified.");
      }
      ...
      if (options->priv_key_file) {
        msg(M_USAGE, "Parameter --key cannot be used when --pkcs11-provider is "
                     "also specified.");
      }
#ifdef ENABLE_MANAGEMENT
      if (options->management_flags & MF_EXTERNAL_KEY) {
        msg(M_USAGE, "Parameter --management-external-key cannot be used when "
                     "--pkcs11-provider is also specified.");
      }
      if (options->management_flags & MF_EXTERNAL_CERT) {
        msg(M_USAGE, "Parameter --management-external-cert cannot be used when "
                     "--pkcs11-provider is also specified.");
      }
#endif
      if (options->pkcs12_file) {
        msg(M_USAGE, "Parameter --pkcs12 cannot be used when --pkcs11-provider "
                     "is also specified.");
      }
#ifdef ENABLE_CRYPTOAPI
      if (options->cryptoapi_cert) {
        msg(M_USAGE, "Parameter --cryptoapicert cannot be used when "
                     "--pkcs11-provider is also specified.");
      }
#endif
    } else
      ...
      if (options->cert_file) {
        ...
        msg(M_USAGE, "--management-external-cert must be used with "
      ...
      if (options->cert_file) {
        msg(M_USAGE, "Parameter --cert cannot be used when --cryptoapicert is "
                     "also specified.");
      }
      if (options->priv_key_file) {
        msg(M_USAGE, "Parameter --key cannot be used when --cryptoapicert is "
                     "also specified.");
      }
      if (options->pkcs12_file) {
        msg(M_USAGE, "Parameter --pkcs12 cannot be used when --cryptoapicert "
                     "is also specified.");
      }
#ifdef ENABLE_MANAGEMENT
      if (options->management_flags & MF_EXTERNAL_KEY) {
        msg(M_USAGE, "Parameter --management-external-key cannot be used when "
                     "--cryptoapicert is also specified.");
      }
      if (options->management_flags & MF_EXTERNAL_CERT) {
        msg(M_USAGE, "Parameter --management-external-cert cannot be used when "
                     "--cryptoapicert is also specified.");
		...
		if (options->pkcs12_file)
		{
			...
			if (options->ca_path)
			{
				msg(M_USAGE, "Parameter --capath cannot be used when --pkcs12 is also "
							 "specified.");
			}
			if (options->cert_file)
			{
      ...
      if (options->management_flags & MF_EXTERNAL_CERT) {
        ...
        if (options->ca_path) {
          msg(M_USAGE, "Parameter --capath cannot be used with the mbed TLS "
                       "version version of OpenVPN.");
        }
              ...
              ((options->priv_key_file != NULL) ||
               (options->management_flags & MF_EXTERNAL_KEY));
				...
				if (sum==0)
				{
					...
					if (!options->auth_user_pass_file)
#endif
					msg(M_USAGE, "No client-side authentication method is specified. "
								 " You must use either --cert/--key, --pkcs12, or "
								 "--auth-user-pass");
				}
				else if (sum==2)
				{
		...
		{
			msg(M_USAGE, "--tls-crypt-v2, --tls-auth and --tls-crypt are mutually "
						 "exclusive in client mode");
		}
	}
	else
	{
...
#define MUST_BE_UNDEF(parm)                                                    \
  if (options->parm != defaults.parm) {                                        \
    msg(M_USAGE, err, #parm);                                                  \
  }

		const char err[] = "Parameter %s can only be specified in TLS-mode, i.e. "
...
static void
options_postprocess_mutate_ce(struct options *o,
							  struct connection_entry *ce)
{
   ...
   */
	if (o->persist_key)
	{
		connection_entry_preload_key(&ce->tls_auth_file, &ce->tls_auth_file_inline,
									 &o->gc);
...
options_postprocess_cipher(struct options *o)
{
	...
	if (!o->ciphername)
	{
		...
		msg(M_WARN,
			"--cipher is not set. Previous OpenVPN version defaulted to "
		...
		!tls_item_in_cipher_list(o->ciphername, o->ncp_ciphers))
	{
		msg(M_WARN,
			...
			o->ciphername, o->ncp_ciphers, o->ciphername, o->ciphername,
			o->ciphername);
...
options_postprocess_mutate(struct options *o)
{
   ...
   */
	...
	if (o->ncp_enabled)
	{
		...
		{
			msg(M_USAGE,
				"NCP cipher list contains unsupported ciphers or is too long.");
		}
	}
...
#define CHKACC_DIRPATH                                                         \
  (1 << 1) /** Check for directory presence where a file should reside */
#define CHKACC_FILEXSTWR (1 << 2) /** If file exists, is it writable? */
#define CHKACC_ACPTSTDIN                                                       \
  (1 << 3) /** If filename is stdin, it's allowed and "exists" */
#define CHKACC_PRIVATE                                                         \
  (1 << 4) /** Warn if this (private) file is group/others accessible */

static bool
check_file_access(const int type, const char *file, const int mode,
				  const char *opt)
{
	int errcode = 0;
   ...
   */
	...
	{
		char *fullpath = string_alloc(
			file,
			NULL); /* POSIX dirname() implementation may modify its arguments */
	...
	if (!errcode && (type & CHKACC_FILEXSTWR) &&
		(platform_access(file, F_OK)==0))
	{
 ...
 * check_file_access() directly, otherwise it will look for the file inside the
 * given chroot directory instead.
 */
static bool
check_file_access_chroot(const char *chroot, const int type,
						 const char *file, const int mode,
						 const char *opt)
{
	bool ret = false;
 ...
 */
static bool
check_file_access_inline(bool is_inline, const int type,
						 const char *file, const int mode,
						 const char *opt)
{
	if (is_inline)
 ...
 * options (e.g., --up) is a valid file with appropriate permissions.
 *
 * "command" consists of a path, optionally followed by a space, which may be
 * followed by arbitrary arguments. It is NOT a full shell command line -- shell
 * expansion is not performed.
 *
 * The path and arguments in "command" may be single- or double-quoted or
 * escaped.
 *
 * The path is extracted from "command", then check_file_access() is called to
 * check it. The arguments, if any, are ignored.
 *
 * Note that the type, mode, and opt arguments to this routine are the same as
 * the corresponding check_file_access() arguments.
 */
static bool
				 ...
				 const char *chroot)
{
     ...
     */
		return_code =
			check_file_access_chroot(chroot, CHKACC_FILE, argv.argv[0], X_OK, opt);
	}
	else
	{
...
options_postprocess_filechecks(struct options *options)
{
	...
	{
		errs |= check_file_access_chroot_inline(
			options->crl_file_inline, options->chroot_dir, CHKACC_FILE,
	...
	for (int i = 0; i < options->connection_list->len; ++i)
	{
		...
		errs |= check_file_access_inline(
			ce->tls_crypt_v2_file_inline, CHKACC_FILE | CHKACC_PRIVATE,
			ce->tls_crypt_v2_file, R_OK, "--tls-crypt-v2");
	}

	errs |= check_file_access_inline(
		options->shared_secret_file_inline, CHKACC_FILE | CHKACC_PRIVATE,
	...
	/* ** System related ** */
	errs |= check_file_access(CHKACC_FILE, options->chroot_dir, R_OK | X_OK,
							  "--chroot directory");
	errs |= check_file_access(CHKACC_DIRPATH | CHKACC_FILEXSTWR,
							  options->writepid, R_OK | W_OK, "--writepid");

	/* ** Log related ** */
	errs |= check_file_access(CHKACC_DIRPATH | CHKACC_FILEXSTWR,
							  options->status_file, R_OK | W_OK, "--status");

	/* ** Config related ** */
	errs |= check_file_access_chroot(options->chroot_dir, CHKACC_FILE,
									 options->tls_export_cert, R_OK | W_OK | X_OK,
									 "--tls-export-cert");
	errs |= check_file_access_chroot(options->chroot_dir, CHKACC_FILE,
									 options->client_config_dir, R_OK | X_OK,
									 "--client-config-dir");
	errs |= check_file_access_chroot(options->chroot_dir, CHKACC_FILE,
									 options->tmp_dir, R_OK | W_OK | X_OK,
...
void pre_pull_save(struct options *o) {
  if (o->pull) {
    ...
    if (o->client_nat) {
      o->pre_pull->client_nat =
          clone_client_nat_option_list(o->client_nat, &o->gc);
 ...
 */
static size_t
calc_options_string_link_mtu(const struct options *o,
...
options_string(const struct options *o, const struct frame *frame,
			   struct tuntap *tt, openvpn_net_ctx_t *ctx, bool remote,
			   struct gc_arena *gc)
{
   ...
   */
	if (!tt)
	{
		tt = init_tun(o->dev, o->dev_type, o->topology, o->ifconfig_local,
					  o->ifconfig_remote_netmask, o->ifconfig_ipv6_local,
					  o->ifconfig_ipv6_netbits, o->ifconfig_ipv6_remote, NULL, NULL,
					  false, NULL, ctx);
		if (tt)
		{
			tt_local = true;
    ...
    buf_printf(&out, ",comp-lzo"); /* for compatibility, this simply indicates
                                      that compression context is active, not
                                      necessarily LZO per-se */
  }
#endif
...
#define TLS_SERVER (o->tls_server)
   ...
   */
	if (o->shared_secret_file || TLS_CLIENT || TLS_SERVER)
	{
		...
		ASSERT((o->shared_secret_file!=NULL) + (TLS_CLIENT==true) +
			(TLS_SERVER==true) <=
			1);
...
static void
options_warning_safe_scan2(const int msglevel, const int delim,
						   ...
						   const char *b2_name)
{
   ...
   */
	if (strprefix(p1, "key-method ") || strprefix(p1, "keydir ") ||
		strprefix(p1, "proto ") || streq(p1, "tls-auth") ||
		strprefix(p1, "tun-ipv6") || strprefix(p1, "cipher "))
	{
		return;
	}
	...
	if (strlen(p1) > 0)
	{
				...
				if (!strcmp(p1_prefix, p2_prefix))
				{
					if (report_inconsistent)
					{
						msg(msglevel,
							...
							safe_print(p1_prefix, &gc), b1_name, safe_print(p1, &gc),
							b2_name, safe_print(p2, &gc));
					}
					goto done;
				}
			}
		...
		msg(msglevel,
			"WARNING: '%s' is present in %s config but missing in %s config, "
			"%s='%s'",
			safe_print(p1_prefix, &gc), b1_name, b2_name, b1_name,
			safe_print(p1, &gc));
...
static void
options_warning_safe_scan1(const int msglevel, const int delim,
						   const bool report_inconsistent,
						   ...
						   const char *b2_name)
{
	...
	{
		options_warning_safe_scan2(msglevel, delim, report_inconsistent, p, b2_src,
								   b1_name, b2_name);
	}
...
static void
options_warning_safe_ml(const int msglevel, char *actual,
						const char *expected, size_t actual_n)
{
	struct gc_arena gc = gc_new();
	...
	if (actual_n > 0)
	{
		...
		options_warning_safe_scan1(msglevel, ',', true, &local, &remote, "local",
								   "remote");

		options_warning_safe_scan1(msglevel, ',', false, &remote, &local, "remote",
								   "local");
	}
...
bool
options_cmp_equal_safe(char *actual, const char *expected,
					   size_t actual_n)
{
	struct gc_arena gc = gc_new();
	bool ret = true;
 ...
 * @return enum windows_driver_type  driver type, WINDOWS_DRIVER_UNSPECIFIED on
 * unknown --windows-driver value
 */
...
print_topology(const int topology)
{
	switch (topology)
	{
		case TOP_UNDEF:return "undef";

		case TOP_NET30:return "net30";

		case TOP_P2P:return "p2p";

		case TOP_SUBNET:return "subnet";
...
int auth_retry_get(void) { return global_auth_retry; }
...
usage(void)
{
	...
	fprintf(fp, usage_message, title_string, o.ce.connect_retry_seconds,
			...
			TUN_MTU_DEFAULT, TAP_MTU_EXTRA_DEFAULT, o.verbosity, o.authname,
			o.ciphername, o.replay_window, o.replay_time, o.tls_timeout,
		   ...
		   const int line_num, int msglevel, struct gc_arena *gc)
{
	...
	do
	{
		...
		else
		{
			...
			if (backslash && out)
			{
              ...
              error_prefix, file, line_num);
					...
					msg(msglevel,
						"%sOptions warning: Bad backslash ('\\') usage in %s:%d: "
						"remember that backslashes are treated as shell-escapes and if "
						"you need to pass backslash characters as part of a Windows "
						"filename, you should use double backslashes such as "
						"\"c:\\\\"
					PACKAGE
					"\\\\static.key\"",
						error_prefix, file, line_num);
		...
		if (out)
		{
			if (parm_len >= SIZE(parm))
			{
				parm[SIZE(parm) - 1] = 0;
				msg(msglevel,
					"%sOptions error: Parameter at %s:%d is too long (%d chars max): "
					"%s",
	...
	{
		msg(msglevel, "%sOptions error: No closing quotation (\") in %s:%d",
			error_prefix, file, line_num);
		return 0;
	}
	if (state==STATE_READING_SQUOTED_PARM)
	{
		msg(msglevel, "%sOptions error: No closing single quotation (\') in %s:%d",
			error_prefix, file, line_num);
		return 0;
	}
	if (state!=STATE_INITIAL)
	{
		msg(msglevel, "%sOptions error: Residual parse state (%d) in %s:%d",
...
add_option(struct options *options, char *p[], bool is_inline,
		   const char *file, int line, const int level,
		   const int msglevel, const unsigned int permission_mask,
		   unsigned int *option_types_found, struct env_set *es);

static void
read_config_file(struct options *options, const char *file,
				 int level, const char *top_file,
				 const int top_line, const int msglevel,
				 ...
				 struct env_set *es)
{
	...
	if (level <= max_recursive_levels)
	{
		...
		if (fp)
		{
			...
			while (fgets(line, sizeof(line), fp))
			{
				...
				{
					msg(msglevel,
						"In %s:%d: Maximum option line length (%d) exceeded, line starts "
						"with %s",
		...
		{
			msg(msglevel, "In %s:%d: Error opening configuration file: %s", top_file,
				top_line, file);
		}
	}
	else
	{
		msg(msglevel,
			"In %s:%d: Maximum recursive include levels exceeded in include "
			"attempt of file %s -- probably you have a configuration file that "
...
static void
read_config_string(const char *prefix, struct options *options,
				   const char *config, const int msglevel,
				   const unsigned int permission_mask,
				   unsigned int *option_types_found,
				   struct env_set *es)
{
	...
	buf_set_read(&multiline, (uint8_t*)
...
void
parse_argv(struct options *options, const int argc, char *argv[],
		   const int msglevel, const unsigned int permission_mask,
		   unsigned int *option_types_found, struct env_set *es)
{
	int i, j;
	...
	else
	{
		...
		for (i = 1; i < argc; ++i)
		{
			...
			{
				msg(msglevel,
					"I'm trying to parse \"%s\" as an --option parameter but I don't "
					"see a leading '--'",
					p[0]);
			}
			else
...
apply_pull_filter(const struct options *o, char *line)
{
	...
	for (f = o->pull_filter_list->head; f; f = f->next)
	{
		...
		else if (f->type==PUF_TYPE_IGNORE &&
		...
		else if (f->type==PUF_TYPE_REJECT &&
			strncmp(line, f->pattern, f->size)==0)
		{
...
void
options_server_import(struct options *o, const char *filename,
					  int msglevel, unsigned int permission_mask,
					  unsigned int *option_types_found,
					  struct env_set *es)
{
	msg(D_PUSH, "OPTIONS IMPORT: reading client specific options from: %s",
		filename);
	read_config_file(o, filename, 0, filename, 0, msglevel, permission_mask,
					 option_types_found, es);
}

void
options_string_import(struct options *options, const char *config,
					  const int msglevel,
					  const unsigned int permission_mask,
					  unsigned int *option_types_found,
					  struct env_set *es)
{
	read_config_string("[CONFIG-STRING]", options, config, msglevel,
					   permission_mask, option_types_found, es);
}
...
static bool verify_permission(const char *name, const char *file, int line,
                              const unsigned int type,
                              const unsigned int allowed, unsigned int *found,
                              const int msglevel, struct options *options,
                              bool is_inline) {
  if (!(type & allowed)) {
    msg(msglevel, "option '%s' cannot be used in this context (%s)", name,
        file);
    return false;
  }
...
static bool
no_more_than_n_args(const int msglevel, char *p[], const int max,
					const unsigned int flags)
{
	...
	if (len > max)
	{
		msg(msglevel, "the --%s directive should have at most %d parameter%s.%s",
			p[0], max - 1, max >= 3 ? "s" : "",
			(flags & NM_QUOTE_HINT)
			? "  To pass a list of arguments as one of the parameters, try "
...
static void
set_user_script(struct options *options, const char **script,
				const char *new_script, const char *type,
				bool in_chroot)
{
	if (*script)
	{
	...
	*script = new_script;
...
#ifndef ENABLE_SMALL
	{
		...
		openvpn_snprintf(script_name, sizeof(script_name), "--%s script", type);

		if (check_cmd_access(*script, script_name,
							 (in_chroot ? options->chroot_dir : NULL)))
		{
		   ...
		   unsigned int *option_types_found, struct env_set *es)
{
   ...
   */
	...
	else if (streq(p[0], "ifconfig") && p[1] && p[2] && !p[3])
	{
		...
		if (ip_or_dns_addr_safe(p[1], options->allow_pull_fqdn) &&
			ip_or_dns_addr_safe(
				p[2], options->allow_pull_fqdn)) /* FQDN -- may be DNS name */
		{
	...
	else if (streq(p[0], "ifconfig-ipv6") && p[1] && p[2] && !p[3])
	{
		...
		if (get_ipv6_addr(p[1], NULL, &netbits, msglevel) && ipv6_addr_safe(p[2]))
		{
			...
			options->ifconfig_ipv6_local =
				get_ipv6_addr_no_netbits(p[1], &options->gc);
			options->ifconfig_ipv6_netbits = netbits;
			options->ifconfig_ipv6_remote = p[2];
		}
		else
		{
			msg(msglevel, "ifconfig-ipv6 parms '%s' and '%s' must be valid addresses",
				p[1], p[2]);
			goto err;
		}
	}
	...
	else if (streq(p[0], "ignore-unknown-option") && p[1])
	{
		...
		for (i = 0;
			 options->ignore_unknown_option && options->ignore_unknown_option[i];
			 i++)
		{
			numignored++;
		}
    ...
    options->http_proxy_override =
        parse_http_proxy_override(p[1], p[2], p[3], msglevel, &options->gc);
	...
	else if (streq(p[0], "remote") && p[1] && !p[4])
	{
		...
		if (p[2])
		{
			...
			if (p[3])
			{
				...
				if (proto < 0)
				{
					msg(msglevel, "remote: bad protocol associated with host %s: '%s'",
						p[1], p[3]);
					goto err;
				}
				re.proto = proto;
	...
	else if (streq(p[0], "ipchange") && p[1])
	{
		...
		set_user_script(options, &options->ipchange,
						string_substitute(p[1], ',', ' ', &options->gc), "ipchange",
						true);
	}
	...
	else if (streq(p[0], "daemon") && !p[2])
	{
		...
		if (p[1])
		{
			...
			{
				msg(M_WARN,
					"WARNING: Multiple --daemon directives specified, ignoring "
					"--daemon %s. (Note that initscripts sometimes add their own "
					"--daemon directive.)",
					p[1]);
				goto err;
			}
	...
	else if (streq(p[0], "inetd") && !p[3])
	{
		...
		if (!options->inetd)
		{
			...
			const char *opterr = "when --inetd is used with two parameters, one of "
								 "them must be 'wait' or 'nowait' and the other must "
								 "be a daemon name to use for system logging";
     ...
     */
		...
		{
			msg(M_WARN,
				"NOTE: debug verbosity (--verb %d) is enabled but this build lacks "
				"debug support.",
				options->verbosity);
		}
#endif
    ...
    if (shaper < SHAPER_MIN || shaper > SHAPER_MAX) {
      msg(msglevel, "Bad shaper value, must be between %d and %d", SHAPER_MIN,
          SHAPER_MAX);
      goto err;
    }
	...
	else if (streq(p[0], "http-proxy") && p[1] && !p[5])
	{
       ...
       */
			/* (auto|semiauto)-nct -- disable proxy auth cleartext protocols (i.e.
       * basic auth) */
			if (streq(p[3], "auto"))
			{
				ho->auth_retry = PAR_ALL;
	...
	else if (streq(p[0], "http-proxy-user-pass") && p[1])
	{
		...
		streq(p[0], "socks-proxy-retry"))
	{
		VERIFY_PERMISSION(OPT_P_GENERAL | OPT_P_CONNECTION);
		msg(M_WARN, "DEPRECATED OPTION: http-proxy-retry and socks-proxy-retry: "
					"In OpenVPN 2.4 proxy connection retries are handled like "
					"regular connections. "
	...
	else if (streq(p[0], "http-proxy-option") && p[1] && !p[4])
	{
		...
		else if ((streq(p[1], "EXT1") || streq(p[1], "EXT2") ||
			streq(p[1], "CUSTOM-HEADER")) &&
			p[2])
		{
       ...
       * with either two argument or one */
		...
		{
			msg(msglevel, "Bad http-proxy-option or missing or extra parameter: '%s'",
				p[1]);
		}
	}
	...
	else if (streq(p[0], "route") && p[1] && !p[5])
	{
		...
		if (pull_mode)
		{
			...
			{
				msg(msglevel, "route parameter network/IP '%s' must be a valid address",
					p[1]);
				goto err;
			}
			if (p[2] &&
				!ip_addr_dotted_quad_safe(p[2])) /* FQDN -- must be IP address */
			{
				msg(msglevel, "route parameter netmask '%s' must be an IP address",
					p[2]);
				goto err;
			}
			...
			{
				msg(msglevel, "route parameter gateway '%s' must be a valid address",
					p[3]);
				goto err;
			}
		}
	...
	else if (streq(p[0], "route-ipv6") && p[1] && !p[4])
	{
		...
		if (pull_mode)
		{
			...
			{
				msg(msglevel,
					"route-ipv6 parameter network/IP '%s' must be a valid address",
					p[1]);
				goto err;
			}
	...
	else if (streq(p[0], "route-ipv6-gateway") && p[1] && !p[2])
	{
		...
		{
			msg(msglevel, "route-ipv6-gateway parm '%s' must be a valid address",
				p[1]);
			goto err;
		}
	}
	...
	else if (streq(p[0], "route-pre-down") && p[1])
	{
		...
		set_user_script(options, &options->route_predown_script, p[1],
						"route-pre-down", true);
	}
    ...
    if (netbits < 64 || netbits > 124) {
      msg(msglevel,
          "--server-ipv6 settings: network must be between /64 and /124 (not "
          "/%d)",
          netbits);
    ...
    {
      msg(msglevel, "error parsing --server-ipv6: %s is not a recognized flag",
          p[3]);
      goto err;
    }
    ...
    if (netbits < 64 || netbits > 124) {
      msg(msglevel,
          "--ifconfig-ipv6-pool settings: network must be between /64 and /124 "
          "(not /%d)",
          netbits);
      goto err;
    }
    ...
    if (p[2]) {
      if (streq(p[2], "via-env")) {
        ...
        msg(msglevel, "second parm to --auth-user-pass-verify must be "
                      "'via-env' or 'via-file'");
        goto err;
      }
    } else {
    ...
    set_user_script(options, &options->client_connect_script, p[1],
                    "client-connect", true);
    ...
    set_user_script(options, &options->client_disconnect_script, p[1],
    ...
    set_user_script(options, &options->learn_address_script, p[1],
                    "learn-address", true);
    ...
    remote_netmask =
        getaddr(GETADDR_HOST_ORDER | GETADDR_RESOLVE, p[2], 0, NULL, NULL);
    ...
    VERIFY_PERMISSION(OPT_P_GENERAL);
    network =
        getaddr(GETADDR_HOST_ORDER | GETADDR_RESOLVE, p[1], 0, NULL, NULL);
    ...
    if (p[2]) {
      ...
      if (!options->ifconfig_ipv6_local ||
          !get_ipv6_addr(options->ifconfig_ipv6_local, &remote, NULL,
                         msglevel)) {
        msg(msglevel, "second argument to --ifconfig-ipv6-push missing and no "
                      "global --ifconfig-ipv6 address set");
        goto err;
      }
    }
      ...
      if (p[2]) {
        if (!streq(p[2], "default")) {
          ...
          if (!(offset > -256 && offset < 256)) {
            msg(msglevel,
                "--ip-win32 dynamic [offset] [lease-time]: offset (%d) must be "
                "> -256 and < 256",
                offset);
            goto err;
          }
        ...
        if (p[3]) {
          ...
          if (lease_time < min_lease) {
            msg(msglevel,
                "--ip-win32 dynamic [offset] [lease-time]: lease time "
                "parameter (%d) must be at least %d seconds",
    ...
    if ((streq(p[1], "DOMAIN") || streq(p[1], "ADAPTER_DOMAIN_SUFFIX")) &&
      ...
      t = atoi(p[2]);
      if (!(t == 1 || t == 2 || t == 4 || t == 8)) {
        msg(msglevel, "--dhcp-option NBT: parameter (%d) must be 1, 2, 4, or 8",
            t);
        goto err;
      }
      ...
      msg(msglevel,
          "--dhcp-option: unknown option type '%s' or missing or unknown "
          "parameter",
          p[1]);
      goto err;
    }
  ...
  } else if (streq(p[0], "dhcp-internal") && p[1] &&
             !p[2]) /* standalone method for internal use */
  {
   ...
   */
  {
    ...
    if (streq(p[1], "no")) {
      ...
      msg(M_WARN,
          "WARNING: Compression for sending and receiving enabled. Compression "
          "has "
          "been used in the past to break encryption. Allowing compression "
          "allows "
...
#endif
    {
      ...
      msg(msglevel,
          ...
          "allow-compression is set to 'no'",
          p[1]);
      goto err;
    } else if (p[1]) {
      if (streq(p[1], "yes")) {
	...
	else if (streq(p[0], "replay-window") && !p[3])
	{
		...
		if (p[1])
		{
			...
			if (!(MIN_SEQ_BACKTRACK <= replay_window &&
				replay_window <= MAX_SEQ_BACKTRACK))
			{
				msg(msglevel,
					"replay-window window size parameter (%d) must be between %d and "
					"%d",
			...
			if (p[2])
			{
				...
				if (!(MIN_TIME_BACKTRACK <= replay_time &&
					replay_time <= MAX_TIME_BACKTRACK))
				{
					msg(msglevel,
						"replay-window time window parameter (%d) must be between %d and "
						"%d",
	...
	else if (streq(p[0], "verify-hash") && p[1] && !p[3])
	{
		...
		{
			options->verify_hash = parse_hash_fingerprint(p[1], SHA256_DIGEST_LENGTH,
														  msglevel, &options->gc);
		...
		{
			msg(msglevel,
				"invalid or unsupported hashing algorithm: %s  (only SHA1 and SHA256 "
				"are valid)",
				p[2]);
			goto err;
		}
	...
	else if (streq(p[0], "xor_secret") && p[1])
	{
		VERIFY_PERMISSION(OPT_P_GENERAL);
		_xor_secret = p[1];
		_xor_secret_len = strlen(_xor_secret);
	}
     ...
     * set warning level to non fatal */
		...
		{
			msg(msglevel,
				"Unrecognized option or missing or extra parameter(s) in %s:%d: %s "
				"(%s)",
