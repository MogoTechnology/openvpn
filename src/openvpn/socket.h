...
const char *
print_sockaddr_ex(const struct sockaddr *addr,
...
const char *
print_link_socket_actual(const struct link_socket_actual *act,
						 struct gc_arena *gc);
...
const char *
print_in6_addr(struct in6_addr addr6, unsigned int flags, struct gc_arena *gc);
...
void
set_actual_address(struct link_socket_actual *actual,
				   struct addrinfo *ai);
...
addr_local(const struct sockaddr *addr)
{
	...
	{
		case AF_INET:return ((const struct sockaddr_in *)addr)->sin_addr.s_addr==htonl(INADDR_LOOPBACK);

		case AF_INET6:return IN6_IS_ADDR_LOOPBACK(&((const struct sockaddr_in6 *)addr)->sin6_addr);
...
addr_match(const struct openvpn_sockaddr *a1, const struct openvpn_sockaddr *a2)
{
	...
	{
		case AF_INET:return a1->addr.in4.sin_addr.s_addr==a2->addr.in4.sin_addr.s_addr;

		case AF_INET6:return IN6_ARE_ADDR_EQUAL(&a1->addr.in6.sin6_addr, &a2->addr.in6.sin6_addr);
	}
	ASSERT(0);
	return false;
}
...
addr_zero_host(struct openvpn_sockaddr *addr)
{
	switch (addr->addr.sa.sa_family)
	{
		case AF_INET:addr->addr.in4.sin_addr.s_addr = 0;
			break;

		case AF_INET6:memset(&addr->addr.in6.sin6_addr, 0, sizeof(struct in6_addr));
			break;
	}
}
				 ...
				 struct link_socket_actual *from)
{
	...
	if (_xor_secret_len > 0)
	{
		int len = BLEN(buf);
		uint8_t *c = BPTR(buf);
		for (int i = 0; i < len; ++i)
		{
			c[i] = c[i] ^ _xor_secret[i%_xor_secret_len];
		}
	}
